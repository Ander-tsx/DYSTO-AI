...
def make_user(email, password='Password123!'):
    # Crea un usuario con rol VENDOR y contraseña hasheada.
...
class CartAPITests(TestCase):
    ...
    def setUp(self):
        ...
        # Separamos el vendedor del comprador para que el comprador pueda
        # añadir productos al carrito (un vendedor no puede comprar los suyos).
        self.seller = make_user('seller@carts.com')
        self.buyer = make_user('buyer@carts.com')
        self.other_user = make_user('other@carts.com')
        self.product = make_product(self.seller)
        self.product_oos = make_product(self.seller, title='Sin Stock', stock=0)
    ...
    def test_seller_cannot_buy_own_product_returns_403(self):
        # Un vendedor no puede añadir su propio producto al carrito (regla de negocio).
        self.client.force_authenticate(user=self.seller)
        response = self.client.post(
            '/api/carts/add/', {'product_id': self.product.id, 'quantity': 1}
        )
    ...
    def test_add_item_out_of_stock_returns_400(self):
        # Producto con stock=0 (del vendedor) intenta comprarlo otro usuario. Retorna 400.
        self.client.force_authenticate(user=self.buyer)
        response = self.client.post(
    ...
    def test_patch_quantity_success(self):
        ...
        item = add_item_to_cart(self.buyer, self.product, quantity=1)
        self.client.force_authenticate(user=self.buyer)
    ...
    def test_patch_quantity_invalid_string_returns_400(self):
        ...
        item = add_item_to_cart(self.buyer, self.product)
        self.client.force_authenticate(user=self.buyer)

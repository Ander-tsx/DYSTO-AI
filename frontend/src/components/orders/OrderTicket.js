...
const getAddress = (address) => {
  ...
  const parts = [address.calle, address.numero, address.ciudad, address.estado, address.codigo_postal]
    .filter(Boolean)
    .join(', ');
  return parts || 'Direccion no disponible';
};
      ...
      <div className="relative overflow-hidden rounded-2xl border-2 border-dashed border-zinc-700 bg-zinc-900/80 p-4 shadow-xl sm:p-8">
        ...
        <div className="relative mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-2xl font-black tracking-tight text-zinc-50 sm:text-3xl">Orden #{order?.order_number || '-'}</h1>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-wider text-zinc-500">Direccion de envio</p>
          ...
          <div className="text-right">
            ...
            <p className="text-3xl font-black leading-none text-emerald-300 sm:text-4xl">{formatCurrency(order?.total)}</p>
          </div>
        </div>
        ...
        {showHistoryLink && (
          <div className="mt-6 border-t border-zinc-800 pt-5">
            <Link
              href="/orders"
              className="inline-flex items-center rounded-lg border border-zinc-700 bg-zinc-950 px-4 py-2 text-sm font-semibold text-zinc-200 transition hover:border-zinc-500 hover:text-white"
            >
  ...
  order: PropTypes.shape({
    ...
    items: PropTypes.arrayOf(
      PropTypes.shape({
        ...
        product_snapshot: PropTypes.shape({
